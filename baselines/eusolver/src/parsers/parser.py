...
from semantics import semantics_obj
...
def sexp_to_value(sexp):
    ...
    if value_type.type_code == exprtypes.TypeCodes.integer_type:
        ...
        value = value_exp
    elif value_type.type_code == exprtypes.TypeCodes.obj_type:
        value = value_exp
    else:
...
def sexp_to_grammar(arg_vars, grammar_sexp, synth_fun, syn_ctx):
    ...
    for nt, nt_type_data, rules_data in grammar_sexp:
        ...
        for phlb in ph_let_bound_vars:
            lvs = [v for v in let_bound_vars if
                   'ph_' + v.variable_info.variable_name == phlb.variable_info.variable_name]
            assert len(lvs) > 0
...
_known_theories = ["LIA", "BV", "SLIA", "OBJ"]
...
def get_theory_instantiator(theory):
    if theory == "LIA":
        ...
        return semantics_obj.OBJInstantiator()
    else:
